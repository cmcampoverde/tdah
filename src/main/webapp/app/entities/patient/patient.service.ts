...
export class PatientService {
  ...
  protected convertDateFromClient(patient: IPatient): IPatient {
    const copy: IPatient = Object.assign({}, patient, {
      birthday: patient.birthday && patient.birthday.isValid() ? patient.birthday.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((patient: IPatient) => {
        patient.birthday = patient.birthday ? moment(patient.birthday) : undefined;
      });
    }
    return res;
  }
